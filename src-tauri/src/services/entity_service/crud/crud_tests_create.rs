...
async fn count_by_campaign_returns_per_kind_counts() {
    ...
    let input = |name: &str| EntityInput { name: name.to_string(), ..Default::default() };
    create(&db, Some("camp1"), None, EntityKind::Npc, input("Torvin")).await.unwrap();
    create(&db, Some("camp1"), None, EntityKind::Npc, input("Mira")).await.unwrap();
    create(&db, Some("camp1"), None, EntityKind::Location, input("Docks")).await.unwrap();
...
async fn count_by_campaign_does_not_count_other_campaigns() {
    ...
    create(
        &db, Some("camp2"), None, EntityKind::Npc,
        EntityInput { name: "Elsewhere".to_string(), ..Default::default() },
    ).await.unwrap();
    let counts = count_by_campaign(&db, "camp1").await.unwrap();
...
async fn create_with_campaign_id_populates_campaign_via_edge() {
    ...
    let node = create(
        ...
        EntityInput { name: "Torvin".to_string(), ..Default::default() },
    ).await.unwrap();
...
async fn create_with_collection_id_populates_collection_via_edge() {
    ...
    let node = create(
        &db, None, Some("col1"), EntityKind::Npc,
        EntityInput { name: "Goblin".to_string(), ..Default::default() },
    ).await.unwrap();
