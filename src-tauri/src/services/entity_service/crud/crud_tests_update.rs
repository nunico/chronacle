...
async fn update_clears_nullable_fields_to_null_not_none() {
    ...
    let updated = update(
        &db, &node.id, EntityKind::Npc,
        EntityInput { name: "Torvin".to_string(), summary: None, notes: None, ..Default::default() },
    ).await.expect("update should not error when clearing nullable fields");
    assert_eq!(updated.summary, None);
    assert_eq!(updated.notes, None);

    let refetched = get_by_id(&db, &node.id, EntityKind::Npc).await.unwrap();
    assert_eq!(refetched.summary, None, "summary should be cleared in the DB");
...
async fn update_clears_nullable_event_fields() {
    let db = setup_db().await;
    let node = create(
        &db, Some("camp1"), None, EntityKind::Event,
        EntityInput {
    ...
    ).await.expect("clearing nullable event fields should not error");
