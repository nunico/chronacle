...
async fn multiple_wikilinks_all_returned() {
    ...
    let ironhold = create(&db, Some(&campaign_id), None, EntityKind::Location, make_npc("Ironhold")).await.unwrap();
    ...
    let result = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "[[Torvin]] traveled to [[Ironhold]] yesterday.",
...
async fn session_source_skips_relates_to_edges_but_returns_ids() {
    ...
    let torvin = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("Torvin")).await.unwrap();
    let result = parse_and_sync_wikilinks(
        &db, "session", "somesessionid", "[[Torvin]] appeared.",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await.unwrap();
    assert_eq!(result, vec![format!("npc:{}", torvin.id)]);
    let mut resp = db.query("SELECT count() FROM relates_to GROUP ALL").await.unwrap();
...
async fn repeated_call_same_notes_produces_single_edge() {
    ...
    let count: Option<serde_json::Value> = resp.take(0).unwrap();
    let n = count.and_then(|v| v.get("count").and_then(|c| c.as_i64())).unwrap_or(0);
    assert_eq!(n, 1, "repeated calls with same notes must produce exactly one edge, got {n}");
    let _ = torvin;
}
...
async fn invalid_source_table_returns_error() {
    ...
    let campaign_id = create_campaign(&db).await;
    let result = parse_and_sync_wikilinks(
        &db, "npc; DROP TABLE npc", "someId", "some notes",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await;
    assert!(
...
fn split_record_id_error_on_missing_colon() {
    ...
    assert!(matches!(result, Err(WikilinkError::MalformedRecordId { .. })));
...
fn entity_tables_matches_entity_kind() {
    ...
    assert_eq!(ENTITY_TABLES.len(), kind_count, "ENTITY_TABLES length doesn't match EntityKind variant count");
}
