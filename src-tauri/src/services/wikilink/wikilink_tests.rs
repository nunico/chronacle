...
async fn case_insensitive_match_returns_entity_id() {
    let db = setup_db().await;
    let campaign_id = create_campaign(&db).await;
    let npc = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("torvin")).await.unwrap();
    ...
    let result = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "We met [[Torvin]] at the inn.",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await.unwrap();
...
async fn stale_relates_to_edge_deleted_on_second_call() {
    ...
    let result2 = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "The inn was empty.",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await.unwrap();
    assert!(result2.is_empty());
    let mut resp2 = db.query("SELECT count() FROM relates_to GROUP ALL").await.unwrap();
    let count2: Option<serde_json::Value> = resp2.take(0).unwrap();
    let n2 = count2.and_then(|v| v.get("count").and_then(|c| c.as_i64())).unwrap_or(0);
...
async fn mentioned_edge_skipped_when_higher_tier_edge_exists() {
    ...
    let torvin = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("Torvin")).await.unwrap();
    let source = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("SourceNPC")).await.unwrap();
    relate(&db, &torvin.id, "npc", &source.id, "npc", "leads", None).await.unwrap();
    let result = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "We met [[Torvin]] at the inn.",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await.unwrap();
