...
async fn duplicate_wikilink_in_notes_produces_single_edge() {
    ...
    let torvin = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("Torvin")).await.unwrap();
    let source = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("SourceNPC")).await.unwrap();
    parse_and_sync_wikilinks(
        &db, "npc", &source.id, "[[Torvin]] met [[Torvin]] again",
    ...
    let n = count.and_then(|v| v.get("count").and_then(|c| c.as_i64())).unwrap_or(0);
    assert_eq!(n, 1, "duplicate wikilinks must produce exactly one edge, got {n}");
}
...
async fn collection_scope_resolves_same_collection_entities() {
    ...
    let npc = create(&db, None, Some(&col_id), EntityKind::Npc, make_npc("Goblin")).await.unwrap();
    let expected_id = format!("npc:{}", npc.id);
    let campaign_id = create_campaign(&db).await;
    create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("Goblin")).await.unwrap();
    let source = create(&db, None, Some(&col_id), EntityKind::Npc, make_npc("SourceNPC")).await.unwrap();
    let result = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "We fought [[Goblin]].",
...
async fn campaign_scope_resolves_subscribed_collection_entities() {
    ...
    .await.unwrap();
    let col_npc = create(&db, None, Some(&col_id), EntityKind::Npc, make_npc("Dungeon Master")).await.unwrap();
    let expected_id = format!("npc:{}", col_npc.id);
    let source = create(&db, Some(&campaign_id), None, EntityKind::Npc, make_npc("Player")).await.unwrap();
    let result = parse_and_sync_wikilinks(
        &db, "npc", &source.id, "Asked the [[Dungeon Master]] for help.",
        WikilinkScope::Campaign { campaign_id: &campaign_id },
    ).await.unwrap();
    assert_eq!(result.len(), 1);
