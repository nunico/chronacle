...
With cloud embedding, the configured remote embedding endpoint receives each piece of searchable text that Chronacle prepares, as applicable: source chunks; entity names, summaries, notes, and compiled Codex articles; session titles and notes; compiled rules; and question or search text during retrieval. These categories are sent when Chronacle needs to embed them, not all together with every request. Local embedding performs this calculation on this computer. This does not make the separate answer provider local. When answering, a remote answer provider receives the question and the context Chronacle supplies: relevant source excerpts; entity names, summaries, notes, and compiled Codex articles; session titles and notes; and compiled rules, as available for that question and campaign. Campaign entities and sessions can be included as full campaign-scoped context rather than relevance-filtered results.
