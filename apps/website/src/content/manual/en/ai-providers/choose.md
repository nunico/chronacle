...
- When an online answer provider is active, Chronacle sends it the question and the answer context Chronacle supplies. Depending on the selected campaign and sources, that can include relevant source excerpts; entity names, summaries, notes, and compiled Codex articles; player names and character class, level, and status; event start and end dates; session numbers, titles, played dates, and notes; and compiled rules. Campaign entities and sessions can be included as full campaign-scoped context rather than relevance-filtered results.
