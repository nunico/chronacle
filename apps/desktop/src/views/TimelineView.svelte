<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      error = i18n.t('timeline.loadFailed');
      events = [];
      sessions = [];
    } finally {
      loading = false;
...
<div class="timeline">
  <div class="toolbar" role="tablist" aria-label={i18n.t('timeline.mode')}>
    <button
      role="tab"
      aria-selected={mode === 'chronicle'}
      class:active={mode === 'chronicle'}
      onclick={() => (mode = 'chronicle')}>{i18n.t('timeline.chronicle')}</button
    >
    <button
      role="tab"
      aria-selected={mode === 'sessions'}
      class:active={mode === 'sessions'}
      onclick={() => (mode = 'sessions')}>{i18n.t('timeline.sessions')}</button
    >
  </div>
  ...
  {:else if mode === 'chronicle'}
    {#if eraGroups.length === 0 && !error}
      <p class="muted">{i18n.t('timeline.noEvents')}</p>
    {:else}
      <ol class="spine">
        {#each eraGroups as group (group.era ?? '__none__')}
          <li class="era">
            <h3 class="era-head">{group.era ?? i18n.t('timeline.unordered')}</h3>
            <ol class="events">
              {#each group.events as e (e.id)}
                ...
                <li class="event">
                  ...
                  {#if e.is_ongoing}<span class="ongoing">{i18n.t('timeline.ongoing')}</span>{/if}
                </li>
              {/each}
            </ol>
          </li>
  ...
  {:else}
    {#if sessions.length === 0 && events.length === 0 && !error}
      <p class="muted">{i18n.t('timeline.noSessionsOrEvents')}</p>
    {:else}
      <ol class="lanes">
        {#each sessionLanes as lane (lane.session?.id ?? '__none__')}
          <li class="lane">
            <h3 class="lane-head">{lane.session?.title ?? i18n.t('timeline.unscheduled')}</h3>
            <ol class="events">
